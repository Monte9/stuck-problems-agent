...
`spec.md` exists and `status: in_progress` at milestone 1. The next run matches decision table row 2: the **generator** executes M1 (build the scoring rubric and fix the hospital sample), writing one artifact to `artifacts/`, then sets `status: awaiting_review`.
