...
2. **One phase per run.** Plan, generate, evaluate, publish, or intake — never more than one. Small, cheap, reviewable runs are the design, not a limitation.
...
Read every `problems/*/state.md`. Pick the active problem: status not `published` or `blocked`; if several qualify, the one least recently run. If no problem qualifies, row 5 is the match. Then execute the **first matching row** and stop:
...
| 5 | No active problem (everything `published` or `blocked`) | **Intake.** Read the Notion page "Stuck Problems Ledger". Pick the newest brief whose topic is not already in `problems/`. Create `problems/<kebab-slug>/problem.md` (brief verbatim, with date and source noted) and a fresh `state.md` (`status: no_spec`, milestone 0, empty run log). Commit `[intake]`. If the Notion tool is unavailable or no new brief exists, exit quietly. |
...
- `[intake] <problem>: pulled from the scout ledger` — FYI, new problem entering the loop
