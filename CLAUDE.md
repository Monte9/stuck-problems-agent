...
Read every `problems/*/state.md`. Pick the active problem: status not `complete` or `blocked`; if several qualify, the one least recently run. Then execute the **first matching row** and stop:
