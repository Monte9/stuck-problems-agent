...
| 5 | No active problem (everything `published` or `blocked`) | **Intake.** Read `ledger/`. Pick the newest brief not already promoted to `problems/`. Move it: `git mv ledger/<file> problems/<kebab-slug>/problem.md`, create a fresh `state.md` (`status: no_spec`, milestone 0, empty run log). Commit `[intake]`. If the ledger has no unpromoted briefs, exit quietly. |
